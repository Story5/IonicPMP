...
import com.cmsv6demo.DeviceActivity;
...
public class VideoView extends ImageView {
	...
	private int mVideoRgb565Length = 0;
	...
	public VideoView(Context context, int index, DeviceActivity deviceActivity) {
	    super(context);
	    mStrLoading = "loading";
	    this.mIndex = index;
...
//				canvas.drawRect(dest, this.mPaint);
     	...
     	synchronized (lockHandle) {
    		if (this.mRealHandle != 0) {
    			...
    			if (!isLoading) {
    				...
    				videoSize[0] = 0;
    				videoSize[1] = 0;
    				if (NetClient.GetRPlayImage(this.mRealHandle, mVideoRgb565Length, mVideoPixel, videoSize, NetClient.GPS_RGB_FORMAT_565) != NetClient.NET_SUCCESS) {
    ...
    private void initVideoBuf(int nWidth, int nHeight) {
		...
		mVideoRgb565Length = mVideoWidth * mVideoHeight * 2;
		mVideoPixel = new byte[mVideoRgb565Length];
